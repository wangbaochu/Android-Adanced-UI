...
package com.open.ui.refinedslidingdrawer.view;
...
public class SlidingDrawer extends ViewGroup {
    ...
    private float mLastMotionPositionY = 0;
    
    /**
     * The bottom offset of the SlidingDrawer layout position
     */
    ...
    /**
     * Sub class must override this function to judge if content view has scrolled to top.
     */
    protected boolean isScrolledToTop() {
        return false;
    }
    
    private boolean mRequestInterceptTouchEvent = false;
    /**
     * Request sliding drawer to intercept touch event immediately.
     */
    ...
    public SlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        mShowContentAfterClose = a.getBoolean(R.styleable.SlidingDrawer_showContentAfterClose, false);
    ...
    protected void onFinishInflate() {
        ...
        //This listener only be triggered by trackball click event. Finger click event always 
        //intercept by this class onInterceptTouchEvent() and dispatched to be handled in onTouchEvent().
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mTracking || mAnimating) {
        	//drawCache(canvas, isVertical, drawingTime);
            drawChild(canvas, mContent, drawingTime);
        } else if (mExpanded) {
            drawChild(canvas, mContent, drawingTime);
        } else {
            if (mShowContentAfterClose && mBottomOffset != 0.0f) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mVertical) {
            int height = heightSpecSize - handle.getMeasuredHeight() - mTopOffset + (mShowContentAfterClose ? 0 : mBottomOffset);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mVertical) {
            ...
            content.layout(0, childTop + childHeight, content.getMeasuredWidth(),
    ...
    private float mActionMoveThreshold = 10.0f;
    @Override
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            mLastMotionPositionY = y;
        }
        if (action == MotionEvent.ACTION_MOVE) {
            isMoveDown =  y - mLastMotionPositionY > mActionMoveThreshold;
            isMoveUp = mLastMotionPositionY - y > mActionMoveThreshold;
        ...
        if (mTracking || mAnimating) {
            return false;
        } else if (!frame.contains((int) x, (int) y)) {
            if (action == MotionEvent.ACTION_MOVE) {
                if (mExpanded) {
                    if (isMoveDown && isScrolledToTop()) {
                        shouldInterceptTouchEvent = true;
                    }
                } else if(!isMoving() && isMoveUp) {
                    shouldInterceptTouchEvent = true;
                }
            }
        } else {
        ...
        if ((action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_MOVE) && shouldInterceptTouchEvent) {
                mTracking = true;
        ...
        if (mTracking) {
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_CANCEL: {
                    ...
                    float velocity = mVertical ? yVelocity : xVelocity;
                    ...
                    if (Math.abs(velocity) < mMaximumTapVelocity) {
                        if (vertical ? (mExpanded && top < mTapThreshold + mTopOffset) ||
                                (!mExpanded && top > mBottomOffset + getBottom() - getTop() -
                                		mHandle.getHeight() - mTapThreshold) :
                                (mExpanded && right > mTopOffset + getRight() - getLeft() - mTapThreshold) ||
                                (!mExpanded && right < mBottomOffset + mHandle.getWidth() + mTapThreshold)) {

                            if (mAllowSingleTap) {
                                //playSoundEffect(SoundEffectConstants.CLICK);
                                if (mExpanded) {
                                    animateClose(vertical ? top : right);
                                } else {
                                    animateOpen(vertical ? top : right);
                                }
                            } else {
                                performFling(vertical ? top : right, velocity, false);
                            }
        ...
        if (mExpanded) {
        	if (mVertical) {
        		if (always || (velocity > mMaximumMajorVelocity ||
        			...
        			// We are expanded and are now going to collapse.
        			...
        			// us to collapse.  Animate back to the expanded position.
        			mAnimatedAcceleration = -mMaximumAcceleration;
        			if (velocity > 0) {
        				mAnimatedVelocity = 0;
        			}
        		...
        		if (always || (velocity < -mMaximumMajorVelocity ||
        				(position < getWidth() / 2) && velocity < mMaximumMajorVelocity)) {
        			// We are expanded and are now going to collapse.
        			mAnimatedAcceleration = -mMaximumAcceleration;
        			if (velocity > 0) {
        				mAnimatedVelocity = 0;
        	...
        	if (mVertical) {
        		if (!always && (velocity > mMaximumMajorVelocity ||
        			...
        			// us to expand.  Animate back to the collapsed position.
        			mAnimatedAcceleration = mMaximumAcceleration;
        			if (velocity < 0) {
        				mAnimatedVelocity = 0;
        			}
    ...
    private void prepareTracking(int position) {
        ...
        if (opening) {
            ...
            mAnimationPosition = (mVertical ? mBottomOffset + getHeight() - mHandle.getHeight() : mBottomOffset + mHandle.getWidth());
            if (mAnimationPosition != (mVertical ? mHandle.getTop() : mHandle.getRight())) {
            ...
            if (position != (mVertical ? mHandle.getTop() : mHandle.getRight())) {
            	moveHandle(position);
            }
        }
    }
    ...
    private void moveHandle(int position) {
        ...
        if (position == EXPANDED_FULL_OPEN) {
            ...
            percent = ((float) handle.getTop())/(this.getHeight() - mTopOffset + mBottomOffset - handle.getHeight());
        }
        updateHandleBackground(percent);
        
        if (mVertical) {
            if (position == EXPANDED_FULL_OPEN) {
                handle.offsetTopAndBottom(mTopOffset - handle.getTop());
                content.layout(0, handle.getBottom(), content.getMeasuredWidth(), handle.getBottom() + content.getMeasuredHeight());
                invalidate();
            } else if (position == COLLAPSED_FULL_CLOSED) {
                handle.offsetTopAndBottom(mBottomOffset + getBottom() - getTop() -
                		handle.getHeight() - handle.getTop());
                content.layout(0, handle.getBottom(), content.getMeasuredWidth(), handle.getBottom() + content.getMeasuredHeight());
                invalidate();
            } else {
                ...
                } else if (deltaY > mBottomOffset + getBottom() - getTop() - handle.getHeight() - top) {
                    deltaY = mBottomOffset + getBottom() - getTop() - handle.getHeight() - top;
                }
                handle.offsetTopAndBottom(deltaY);
                
                int height = getBottom() - mHandle.getBottom();
            ...
            if (position == EXPANDED_FULL_OPEN) {
                handle.offsetLeftAndRight(mTopOffset + getRight() - getLeft() - handle.getRight());
                content.layout(handle.getRight(), 0, getWidth(), getHeight());
                invalidate();
            } else if (position == COLLAPSED_FULL_CLOSED) {
                handle.offsetLeftAndRight(mBottomOffset + handle.getWidth() - handle.getRight());
                content.layout(handle.getRight(), 0, getWidth(), getHeight());
                invalidate();
            } else {
                final int right = handle.getRight();
                int deltaX = position - right;
                if (position < mBottomOffset + handle.getWidth()) {
                    deltaX = mBottomOffset + handle.getWidth() - right;
                } else if (position > getRight() - getLeft() + mTopOffset) {
                    deltaX = getRight() - getLeft() + mTopOffset - right;
                }
                handle.offsetLeftAndRight(deltaX);
                
                int width = getRight() - mHandle.getRight();
                content.measure(MeasureSpec.makeMeasureSpec(width, MeasureSpec.EXACTLY),
                        MeasureSpec.makeMeasureSpec(getBottom() - getTop(), MeasureSpec.EXACTLY));
                content.layout(mHandle.getRight(), 0, getWidth(), getHeight());
...
//    	if (content.isLayoutRequested()) {
    		if (mVertical) {
    			...
    			int height = getBottom() - getTop() - childHeight - mTopOffset + (mShowContentAfterClose ? 0 : mBottomOffset);
    			mContent.measure(MeasureSpec.makeMeasureSpec(getRight() - getLeft(), MeasureSpec.EXACTLY),
    					MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY));
    			//mContent.layout(0, mTopOffset + childHeight, mContent.getMeasuredWidth(),
    			//		mTopOffset + childHeight + mContent.getMeasuredHeight());
...
//    	if (!content.isHardwareAccelerated()) content.buildDrawingCache(); //API level 11
//    	content.buildDrawingCache();
    ...
    private void doAnimation() {
        if (mAnimating) {
            incrementAnimation();
            if (mVertical) {
            	if (mAnimationPosition >= mBottomOffset + getHeight() - mHandle.getHeight()) {
            		mAnimating = false;
            		closeDrawer();
            	...
            	if (mAnimationPosition <= mBottomOffset + mHandle.getWidth()) {
            		mAnimating = false;
            		closeDrawer();
            	} else if (mAnimationPosition >= mTopOffset + getWidth()) {
            		mAnimating = false;
            		openDrawer();
            	} else {
            		moveHandle((int) mAnimationPosition);
    ...
    public void animateClose() {
        ...
        animateClose(mVertical ? mHandle.getTop() : mHandle.getRight());
    ...
    public void animateOpen() {
        ...
        animateOpen(mVertical ? mHandle.getTop() : mHandle.getRight());
    ...
    private void animateClose(int position) {
        ...
        performFling(position, mVertical ? mMaximumAcceleration : -mMaximumAcceleration, true);
    }
     ...
     *
     * @return True if the drawer is closed, false otherwise.
     */
    ...
    /**
     * Sub-class should override this function to set the expected background. 
     * @param percent the handle's position percent relative to parent. 
     * E.g: 
