...
package com.open.ui.refineddraglistview.view;
...
public class DragSortListView extends ListView {
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            DragSortItemView wrappedItemView;
            View child;
            // Log.d("mobeta",
            // "getView: position="+position+" convertView="+convertView);
            if (convertView != null) {
                wrappedItemView = (DragSortItemView) convertView;
                View oldChild = wrappedItemView.getChildAt(0);
                ...
                if (child != oldChild) {
                    ...
                    // properly
                    if (oldChild != null) {
                        wrappedItemView.removeViewAt(0);
                    }
                    wrappedItemView.addView(child);
                }
            } else {
                child = mAdapter.getView(position, null, DragSortListView.this);
                if (child instanceof Checkable) {
                    wrappedItemView = new DragSortItemViewCheckable(getContext());
                } else {
                    wrappedItemView = new DragSortItemView(getContext());
                }
                
                ViewGroup.LayoutParams params = (ViewGroup.LayoutParams) child.getLayoutParams();
                if (params == null) {
                    wrappedItemView.setLayoutParams(new AbsListView.LayoutParams(
                            ViewGroup.LayoutParams.MATCH_PARENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT));
                    wrappedItemView.addView(child);
                } else {
                    wrappedItemView.addView(child, params);
                }
            }

            // Set the correct item height given drag state; passed
            // View needs to be measured if measurement is required.
            adjustItem(position + getHeaderViewsCount(), wrappedItemView, true);

            return wrappedItemView;
        }
    }
