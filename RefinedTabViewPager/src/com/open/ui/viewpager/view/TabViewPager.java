 ...
 * {@link TabViewPagerAdapter} to generate the pages that the view shows.
 *
 ...
 * 
 * Revised by williamlin 01-18-2013
 * add method disableInterceptTouchEvent(boolean disable) 
 * to solve the conflict with ScrollView or Gallery
 */
public class TabViewPager extends ViewGroup {
    ...
    /**
     * ViewPager 优化 added by williamlin 2013-08-08
     */
    ...
    private TabViewPagerAdapter.DataSetObserver mObserver;
    ...
    private boolean mHasTrulyChangeX;
    /**
    ...
    public interface OnPageChangeListener {
         ...
         * @see TabViewPager#SCROLL_STATE_IDLE
         * @see TabViewPager#SCROLL_STATE_DRAGGING
         * @see TabViewPager#SCROLL_STATE_SETTLING
         */
    ...
    public void setAdapter(TabViewPagerAdapter adapter) {
        if (mAdapter != null) {
    ...
    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always) {
    	//Log.i(TAG, "setCurrentItemInternal item: " + item + " smoothScroll: " + smoothScroll + " always: " +always);
    ...
    void addNewItem(int position, int index) {
    	//Log.i(TAG, "addNewItem position: " + position + " index: " + index);
    ...
    void dataSetChanged() {
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if (newPos == TabViewPagerAdapter.POSITION_UNCHANGED) {
                continue;
            }

            if (newPos == TabViewPagerAdapter.POSITION_NONE) {
                mItems.remove(i);
                i--;
                needPopulate = true;
    ...
    // 强制更新一下子view
    public void updataChildPage(View childView)
    {
    	ItemInfo itemInfo = infoForChild(childView);
    	if (itemInfo != null)
    	{
        	itemInfo.hasLayouted = false;
        	itemInfo.hasMeasure = false;
    ...
    void populate() {
        ...
        if (mOptimize){
        	if (mCurItem >= 0 && mCurItem < mAdapter.getCount()){
        		...
        		if (!added){
        			addNewItem(mCurItem, -1);
        		}
        	}else {
        		//Log.d(TAG, "OPTIMIZE mCurItem: " + mCurItem + " out of bounds");
        	}
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    	...
    	//Log.i(TAG,"optimize onMeasure "+ widthMeasureSpec+"   "+heightMeasureSpec);
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (mOptimize){
            	...
            	if (child.getVisibility() != GONE && ii != null && ((ii.position == mCurItem) || (!ii.hasMeasure))) 
            	{
            		//if (DEBUG) Log.v(TAG, "OPTIMIZE Measuring #" + i + " mCurItem: " + mCurItem + " " + child
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
    	//Log.i(TAG, "optimize onLayout changed: " + changed + " l: " + l + " t: " + t + " r: " + r + " b: " + b);
        mInLayout = true;
        populate();
        mInLayout = false;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mOptimize){
            	...
            	if (child.getVisibility() != GONE && ii != null && ((ii.position == mCurItem) || (!ii.hasLayouted))) 
            	{
            		int loff = width*ii.position;
            		int childLeft = getPaddingLeft() + loff;
            		int childTop = getPaddingTop();
            		//if (DEBUG) Log.v(TAG, "OPTIMIZE onLayout Positioning #" + i + " mCurItem: " + mCurItem +" " + child + " f=" + ii.object
            	...
            	if (child.getVisibility() != GONE && (ii=infoForChild(child)) != null) {
            		...
            		int childLeft = getPaddingLeft() + loff;
            		int childTop = getPaddingTop();
            		//if (DEBUG) Log.v(TAG, "onLayout Positioning #" + i + " " + child + " f=" + ii.object
            		//		+ ":" + childLeft + "," + childTop + " " + child.getMeasuredWidth()
            		//		+ "x" + child.getMeasuredHeight());
    ...
    /** 
     * Flags to indicate ViewPager intercept touch event function disable
     * if true, pass all the touch motion events the  to child view
     */
    private boolean mDisableIntercept = false;

    /**
     * Disable ViewPager intercept touch event function
     * @param disable
     * 			true for disable
     */
    public void disableInterceptTouchEvent(boolean disable){
    	mDisableIntercept = disable;
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (mOnPageChangeListener != null) {
                        ...
                        mOnPageChangeListener.onPageScrollDeltaX(deltaX);
                        mOnPageChangeListener.onPageScrolled(position, positionOffset,
                                positionOffsetPixels);
                        
                    }
                    //if (DEBUG) Log.v(TAG, "Moved mIsBeingDragged " +  "deltaX  " + deltaX+" scrollX  " + scrollX+ "width  " + width );
                }
                break;
    ...
    private class DataSetObserver implements TabViewPagerAdapter.DataSetObserver {
        @Override
